...
do
	fileName=$(printf "vga_%02d_%02d.mp4" ${panels[c]} ${nodes[c]})
  	...
  	echo $fileName;
	cmd=$(printf "$WGET -T 120 -t 2 $mO vgaVideos/$fileName http://domedb.perception.cs.cmu.edu/webdata/dataset/$datasetName/videos/vga_shared_crf10/$fileName || rm -v vgaVideos/$fileName")
	eval $cmd
done
...
$WGET $mO vgaPose3d_stage1_coco19.tar  http://domedb.perception.cs.cmu.edu/webdata/dataset/$datasetName/vgaPose3d_stage1_coco19.tar || rm -v vgaPose3d_stage1_coco19.tar 
fi
